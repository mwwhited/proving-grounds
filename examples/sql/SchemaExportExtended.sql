-- Extended Database Schema Export with Metadata
-- Exports schema with extended properties and ordinal positions
-- Platform: SQL Server
...
SELECT
	...
	,ROW_NUMBER() OVER (ORDER BY [name]) AS [@ordinal]
	,(
		SELECT
			...
			,(
				SELECT
					...
					,(
						SELECT
							[extended_properties].[name] AS [@name]
							,[extended_properties].[value] AS [@value]
						FROM [sys].[extended_properties]
						WHERE
							[extended_properties].[major_id] = [tables].[object_id]
							AND [extended_properties].[minor_id] = [columns].[column_id]
							AND [extended_properties].[value] IS NOT NULL
						FOR XML PATH('ext-prop'), TYPE
					)
				FROM [sys].[columns]
