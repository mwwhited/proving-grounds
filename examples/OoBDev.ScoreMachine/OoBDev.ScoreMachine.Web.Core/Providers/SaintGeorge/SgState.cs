...
namespace OoBDev.ScoreMachine.Web.Core.Providers.SaintGeorge
{
    ...
    {
        public static readonly SgState Empty = new SgState(
            default(byte[]),
            default(SgState),
            default(Fencer),
            default(Fencer),
        ...
        public static SgState Create(SgState last, byte[] frame)
        {
                ...
                frame[3] == 0x02)
            {
                ...
                var scores = parts[1].Select(i => (byte)(i - (byte)'0'))
                                     ...
                                     .ToArray();
                Cards? greenCards = null; // parts.Length < 3 ? (Cards?)null : (Cards)(parts[2][1] - (byte)'0');
                Cards? redCards = null; // parts.Length < 4 ? (Cards?)null : (Cards)(parts[3][1] - (byte)'0');

                var red = new Fencer(scores[1], redCards ?? last?.Red.Cards ?? Cards.None, last?.Red.Lights ?? Lights.None, last?.Red.Priority ?? false);
                var green = new Fencer(scores[0], greenCards ?? last?.Green.Cards ?? Cards.None, last?.Green.Lights ?? Lights.None, last?.Green.Priority ?? false);
